...
export default function VideoMeetingComponent() {
  ...
  let [newMessages, setNewMessage] = useState();
  ...
  useEffect(() => {
    getPermission();
    console.log("Got local media", window.localStream);
  }, [])
  ...
  try {
    ...
    console.log("Error stopping existing stream:", error);
  }
  ...
  let addMessage = (data, sender, socketIdSender) => {
    ...
    if(socketIdSender !== socketIdRef.current){
        setNewMessage((preMessages) => preMessages + 1)
    }
  }
 ...
 let getDispalyMediaSuccess = (stream) =>{
  try{
    window.localStream.getTracks().forEach(track => track.stop())
  }catch (e) {console.log(e)}
  ...
  for(let id in connections){
   ...
   stream.getTrack().forEach( track => track.opended = () =>{
    setScreen(false);
  ...
  let handleEndCall = () =>{
    try{
      let trakcs = localVideoRef.current.srcObject.getTracks();
      trakcs.forEach(track => track.stop());
    }catch(e) {console.log(e)}
            ...
            {showModel ? <div className="chatRoom">
               <div className="chatContainer">
                 ...
                 <div className="chattingArea">
                   <TextField value={message} onChange={e => setMessage(e.target.value)} label="strat your chat" variant="outlined" />
